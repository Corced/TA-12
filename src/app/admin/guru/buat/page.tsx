...
import { createGuru } from "@/api/admin/guru"
...
export default function CreateGuruPage() {
    ...
    const validateForm = () => {
        ...
        const requiredFields = ['kode_guru', 'nama', 'nip', 'password']
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await createGuru(guruData)

            if (response) {
                toast.success('Data guru berhasil dibuat!')
                router.push('/admin/guru')
            } else {
                toast.error('Gagal membuat data guru')
            }
        } catch (error) {
            console.error('Create guru error:', error)
            toast.error('Terjadi kesalahan saat membuat data guru')
        } finally {
            setLoading(false)
        }
    }
