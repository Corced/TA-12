...
import {
  getRealisasiKegiatanPklById,
  updateRealisasiKegiatanPkl,
} from "@/api/pembimbing";
import { uploadImages } from "@/api/upload";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import {
  PostRealisasiKegiatanPkl,
  UpdateRealisasiKegiatanPkl,
} from "@/types/api";
...
export default function EditBuktiKegiatan() {
  ...
  const handleImageChange = async (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const invalidFiles = chosenFiles.filter(
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (images.length > 0) {
        ...
        uploadedUrls = resulUploadImages.results.map(
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">
              Perbarui foto dan catatan kegiatan
            </p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="border-2 border-dashed rounded-xl py-8 flex flex-col items-center text-gray-500 px-4 bg-gray-50/50 hover:bg-gray-50 transition-colors">
                <div className="w-10 h-10 bg-gray-200 rounded-full mb-3 flex items-center justify-center text-xl">
                  📸
                </div>
                <p className="mb-4 text-center text-sm">
                  Seret & lepaskan file atau klik untuk Jelajahi
                </p>
                      ...
                      {preview.progress === 100 ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full p-4 gap-2">
                          ...
                          <span className="text-xs text-center font-medium">
                            Compressing... {Math.round(preview.progress)}%
                          </span>
                        </div>
                      )}
                    </div>
